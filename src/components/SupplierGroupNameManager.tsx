...
interface SupplierGroupNameManagerProps {
  group: SupplierGroup;
  onUpdate: () => void;
  open?: boolean;
...
export const SupplierGroupNameManager: React.FC<SupplierGroupNameManagerProps> = ({
  group,
  onUpdate,
  open: controlledOpen,
  onOpenChange
  ...
  const generateAIName = async () => {
    try {
      ...
      const { data: unitNames, error: unitsError } = await supabase.rpc('get_protheus_supplier_group_unit_names', {
        p_table_id: PROTHEUS_TABLES.SA2010_FORNECEDORES,
        p_filial: group.a2_filial,
        p_cod: group.a2_cod
      });
      ...
      const contextInfo = {
        group_id: `${group.a2_filial}-${group.a2_cod}`,
      ...
      const { data: aiData, error: aiError } = await supabase.functions.invoke('ai-suggest', {
        body: {
          ...
          task: 'extract',
          instructions: `Analise os nomes das unidades empresariais e extraia o nome da marca/empresa principal que representa este grupo de fornecedores.

OBJETIVO: Identificar a marca/razão social principal, não criar um nome genérico.

REGRAS IMPORTANTES:
- Use os nomes reais das unidades como base
- Identifique padrões nos nomes (ex: "Unilever" aparece em várias unidades)
- Evite termos genéricos como "Grupo", "Unificado", "Corporação"
- Prefira nomes de 1-3 palavras
- Se houver uma marca clara, use-a (ex: se várias unidades têm "Unilever", sugira "Unilever")

EXEMPLO:
Se as unidades são: "Unilever Brasil S.A.", "Unilever Nordeste Ltda", "Unilever SP"
Sugestão correta: "Unilever"
  ...
  const saveGroupName = async () => {
    try {
      ...
      const { error } = await supabase
        ...
        .upsert({
          protheus_table_id: PROTHEUS_TABLES.SA2010_FORNECEDORES,
          filial: group.a2_filial,
          cod: group.a2_cod,
          name: finalName,
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          <div className="text-sm text-muted-foreground">
            <strong>Grupo:</strong> {group.a2_filial} - {group.a2_cod} ({group.unit_count} unidades)
          </div>
