...
interface BuyerUserLinkModalProps {
  open: boolean;
...
export const BuyerUserLinkModal = ({ open, onOpenChange, buyer }: BuyerUserLinkModalProps) => {
  const [selectedUserId, setSelectedUserId] = useState<string>('');
  const { profiles, loading: profilesLoading } = useProfiles();
  const { createMutation, updateMutation, deleteMutation, getLinkByBuyerCode } = useBuyerUserLinks();

  const buyerCode = buyer.y1_user?.trim() || '';
  const existingLink = getLinkByBuyerCode(buyerCode);
  const isLinked = !!existingLink;

  const handleSubmit = async () => {
    if (!buyerCode) {
      toast.error('Código do comprador não encontrado');
      return;
    }
    ...
    try {
      if (isLinked && !selectedUserId) {
        ...
        toast.success('Comprador vinculado com sucesso');
      }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-muted/30 p-3 rounded-md">
            <h4 className="text-sm font-medium text-muted-foreground mb-2">Comprador</h4>
            <div className="space-y-1">
              <p className="text-sm font-medium">{buyer.y1_nome || 'Nome não disponível'}</p>
