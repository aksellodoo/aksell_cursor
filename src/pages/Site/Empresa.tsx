...
import { QualityPolicyBanner } from "@/components/site/QualityPolicyBanner";
import { FSSC22000Banner } from "@/components/site/FSSC22000Banner";
import { StructureSection } from "@/components/site/StructureSection";
const Empresa = () => {
  // SEO
  useEffect(() => {
    document.title = "A Empresa - Aksell";

    const desc = "A Aksell é fabricante de sais inorgânicos e híbridos desde 1999. Referência em sais de ferro e outros minerais.";
    ...
    ld.text = JSON.stringify({
      ...
      '@type': 'AboutPage',
      name: 'A Empresa - Aksell',
      url: window.location.href,
      primaryImageOfPage: {
        '@type': 'ImageObject',
        contentUrl: 'https://i.postimg.cc/VzqcvJL5/fachada-aksell.jpg',
        description: 'Fachada da Aksell, Indaiatuba, SP'
      },
      isPartOf: {
        '@type': 'WebSite',
        name: 'Aksell',
  ...
  // Contato (mesmo cabeçalho da home)
  const AREAS = [
...
const { ref: faleRef, inView: faleInView } = useInView({ threshold: 0.2, once: false });
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      const range = 20; // intensidade do parallax
  ...
  return (
    <div id="top">
      {/* Cabeçalho sem borda, igual à home */}
        ...
        <DialogContent className="max-w-5xl w-[96vw] p-0 overflow-hidden">
          <div className="grid md:grid-cols-2 gap-8 items-center">
            ...
            <article className="bg-card/95 text-foreground rounded-[24px] md:rounded-[32px] p-6 md:p-8 shadow-elegant ring-1 ring-primary/10 m-4">
              <h3 className="font-oswald text-primary text-3xl md:text-4xl leading-tight mb-3">Fale com nosso time!</h3>
      ...
      <main>
            ...
            <div className="grid md:grid-cols-2 gap-10 items-center py-2">
              <div className="bg-hero-left rounded-3xl p-3 md:p-6 space-y-4">
                <h1 className="font-oswald text-3xl md:text-5xl leading-tight text-foreground">
                  A Aksell é fabricante de sais inorgânicos e híbridos desde janeiro de 1999.
                </h1>
                <p className="text-muted-foreground max-w-prose">
                  Ao longo de sua história, a Aksell se tornou referência na produção de sais de ferro e de outros minerais para a indústria alimentícia.
                </p>
              </div>
              <div className="relative h-[240px] md:h-[320px] lg:h-[380px] w-full overflow-hidden rounded-3xl shadow-elegant">
                <img
                  ...
                  alt="Fachada da Aksell em Indaiatuba, São Paulo"
                  className="absolute inset-0 w-full h-full object-cover"
                  loading="lazy"
                  referrerPolicy="no-referrer"
                  onError={(e) => {
          ...
          <ResponsiveContainer className="py-10 md:py-14">
            <article className="prose prose-neutral max-w-none text-foreground">
              <p className="first-letter:text-6xl first-letter:font-oswald first-letter:text-primary first-letter:leading-[0.8] first-letter:mr-2 first-letter:float-left">
                Com unidades para atender as indústrias alimentícias e outras unidades para atendimento de diversos outros segmentos, a Aksell está em Indaiatuba (25 km de Campinas e a 95 km de São Paulo), num dos melhores pontos logísticos do Brasil, a Aksell tem expertise na fabricação de sais de alta pureza, atendendo as especificações de cada cliente e aos padrões de qualidade internacionais.
              </p>
              <p>
                Ao longo de sua história, a Aksell se tornou referência na produção de sais de ferro e de outros minerais para a indústria alimentícia, tornando-se fornecedora de grandes empresas com atuação no Brasil e no exterior, fornecendo nossos ingredientes e minerais para vários países.
              </p>
              <p>
                Consulte-nos sobre sua demanda. Temos vários canais de comunicação e estamos prontos a lhe atender.
              </p>
            </article>
          </ResponsiveContainer>
        </section>

        {/* Banner "Missão" com parallax */}
        <section className="bg-background">
          <ResponsiveContainer className="py-6 md:py-10">
            ...
            >
              ...
              <div className="relative grid md:grid-cols-[0.8fr_1.2fr] gap-6 md:gap-10 items-center px-6 md:px-10 py-12 md:py-16">
                <h2 className="font-oswald text-primary text-4xl md:text-6xl leading-none">Missão</h2>
                <p className="text-foreground text-lg md:text-xl max-w-prose">
                  Criar valor duradouro ao produzir e comercializar, de forma responsável, sais e ingredientes alimentícios — com qualidade, segurança e parcerias de confiança.
                </p>
              </div>
            </div>
          </ResponsiveContainer>
        ...
        {/* Fale com nosso time! */}
        <section aria-label="Fale com nosso time" ref={faleRef} className="bg-background">
