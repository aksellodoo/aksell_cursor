...
// Types based on Supabase structure
type ProductSegment = {
  id: string;
  created_at: string;
  name: string;
...
// Product type based on actual table structure
type Product = {
  id: string;
  created_at: string;
  name_id: string;
...
const ProductsPageEN = () => {
  // SEO Setup
  useEffect(() => {
    document.title = "Our Products - Aksell";

    const desc = "Discover our complete line of high-quality chemical products. Inorganic salts, hybrids and specialty ingredients for the food industry.";
    ...
    ld.text = JSON.stringify({
      ...
      mainEntity: {
        '@type': 'ItemList',
        name: 'Aksell Products',
        description: 'Complete line of inorganic salts and specialty ingredients'
      }
    });
  ...
  const AREAS = [
    { value: "sales", label: "Sales", extension: "801", email: "sales@aksell.com.br" },
    { value: "support", label: "Technical Support", extension: "801", email: "support@aksell.com.br" },
    { value: "quality", label: "Quality", extension: "805", email: "quality@aksell.com.br" },
    { value: "financial", label: "Finance", extension: "803", email: "finance@aksell.com.br" },
    { value: "purchasing", label: "Purchasing", extension: "802", email: "purchasing@aksell.com.br" },
    { value: "lab", label: "Lab / Development", extension: "809", email: "lab@aksell.com.br" },
    { value: "hr", label: "Human Resources", extension: "804", email: "hr@aksell.com.br" },
  ] as const;
  const [areaValue, setAreaValue] = useState<string>("sales");
  ...
  const productHeroImages = [
    { src: cosmeticoImg, alt: "Cosmetics and personal care products" },
    { src: embutidosImg, alt: "Sausages and meat products" },
    { src: colheitaImg, alt: "Agricultural products and harvest" },
    { src: dairyImg, alt: "Dairy products and milk derivatives" },
    { src: paesImg, alt: "Bakery and confectionery products" },
    { src: vacasImg, alt: "Livestock and bovine products" },
    { src: hotdogImg, alt: "Processed products and fast food" },
    { src: fabricaImg, alt: "Paper and cellulose industry" }
  ];
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      if (searchTerm && productName) {
        ...
        const displayName = productName.name_en || productName.name;
      ...
      // Segment filter
      if (selectedSegment && selectedSegment !== 'all' && !product.segments?.some(seg => seg?.id === selectedSegment)) {
        return false;
      }

      // Group filter
      if (selectedGroup && selectedGroup !== 'all' && !product.groups?.some(group => group?.id === selectedGroup)) {
        return false;
      }

      // Family filter
      if (selectedFamily && selectedFamily !== 'all' && product.family_id !== selectedFamily) {
        return false;
      }
  ...
  const hasActiveFilters = selectedSegment !== '' && selectedSegment !== 'all' || 
  ...
  const toggleProductExpansion = (productId: string) => {
    ...
    if (newExpanded.has(productId)) {
      ...
      // Also remove from expanded details
  ...
  // Function to format physical state
  ...
  return (
    <div id="top">
          ...
          <div className="hidden md:flex items-center gap-4">
            <NavigationMenu>
              <NavigationMenuList>
                ...
                <NavigationMenuItem>
                  <NavigationMenuLink href="/site/empresa/en" className="px-3 py-2 text-primary hover:text-primary">The Company</NavigationMenuLink>
                </NavigationMenuItem>
              </NavigationMenuList>
            </NavigationMenu>

            <NavLink to="/site/home/pt" aria-label="Switch to Portuguese" title="Português" className="inline-flex">
          ...
          <div className="grid md:grid-cols-2 gap-8 items-center">
            <div className="relative h-[260px] md:h-[420px] lg:h-[520px] overflow-hidden">
              <img
                ...
                alt="Aksell service team at the call center"
            ...
            <article className="bg-card/95 text-foreground rounded-[24px] md:rounded-[32px] p-6 md:p-8 shadow-elegant ring-1 ring-primary/10 m-4">
              <h3 className="text-h2-premium text-primary mb-3 text-hover-lift">Contact our team!</h3>
              <p className="text-body-premium text-muted-foreground mb-6">
                Composed of technicians, chemists and engineers, we are always ready to answer your questions or requests.
              </p>
              <div className="space-y-4">
                <label className="text-sm text-muted-foreground block">Which area would you like to contact?</label>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <a href={`tel:${phoneBase.replace(/[^0-9]/g, '')},${selected.extension}`} className="text-foreground underline hover:text-primary transition">
                    {phoneBase}
                  </a>
      ...
      <main>
        ...
        <section className="bg-gradient-section-warm py-4 md:py-6 -mt-3 md:-mt-4 lg:-mt-5">
          <ResponsiveContainer className="flex items-center justify-center gap-3">
            <div className="w-full max-w-2xl">
              <label className="sr-only" htmlFor="search">Search for product name, molecular formula or CAS</label>
              <div className="group relative flex items-center rounded-full border bg-card/90 backdrop-blur-sm pl-4 pr-2 py-2 shadow-elegant ring-1 ring-primary/10 focus-within:ring-2 focus-within:ring-primary/20 transition-all duration-300">
                <input
                  id="search"
                  placeholder="Search for product name, molecular formula or CAS"
                  value={searchTerm}
                  ...
                  aria-label="Search for product name, molecular formula or CAS"
                />
                <button
                  type="button"
                  aria-label="Search"
            ...
            <div className="flex flex-col lg:flex-row gap-8">
                ...
                <div className="lg:sticky lg:top-24 space-y-4">
                        ...
                        <div className="flex flex-wrap gap-1">
                          {selectedSegment && selectedSegment !== 'all' && (
                            <Badge variant="outline" className="text-xs">
                              {segments.find(s => s.id === selectedSegment)?.name_en || segments.find(s => s.id === selectedSegment)?.name}
                              <button 
                          ...
                          {selectedGroup && selectedGroup !== 'all' && (
                            <Badge variant="outline" className="text-xs">
                              {groups.find(g => g.id === selectedGroup)?.name_en || groups.find(g => g.id === selectedGroup)?.name}
                              <button 
                          ...
                          {selectedFamily && selectedFamily !== 'all' && (
                            <Badge variant="outline" className="text-xs">
                              {families.find(f => f.id === selectedFamily)?.name_en || families.find(f => f.id === selectedFamily)?.name}
                              <button 
                  ...
                  <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <h4 className="font-medium text-foreground">Segments</h4>
                    </div>
                    <div className="space-y-2">
                      <button
                        onClick={() => setSelectedSegment('all')}
                        ...
                        {segments
                          ...
                          .map(segment => {
                            ...
                            return (
                              ...
                              >
                                <span>{segment.name_en || segment.name}</span>
                                <Badge variant="secondary" className="text-xs">
                                  {count}
                                </Badge>
                              </button>
                  ...
                  <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="w-1 h-6 bg-gradient-primary rounded-full"></div>
                      <h4 className="font-medium text-foreground">Groups</h4>
                    </div>
                    <div className="space-y-2">
                      <button
                        onClick={() => setSelectedGroup('all')}
                        ...
                        {groups
                          ...
                          .map(group => {
                            ...
                            return (
                              ...
                              >
                                <span>{group.name_en || group.name}</span>
                                <Badge variant="secondary" className="text-xs">
                                  {count}
                                </Badge>
                              </button>
                  ...
                  <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                    <div className="flex items-center gap-2 mb-3">
                      <div className="w-1 h-6 bg-gradient-primary rounded-full"></div>
                      <h4 className="font-medium text-foreground">Families</h4>
                    </div>
                    <div className="space-y-2">
                      <button
                        onClick={() => setSelectedFamily('all')}
                        ...
                        {families
                          ...
                          .map(family => {
                            ...
                            return (
                              ...
                              >
                                <span>{family.name_en || family.name}</span>
                                <Badge variant="secondary" className="text-xs">
                                  {count}
                                </Badge>
                              </button>
              ...
              <div className="lg:w-3/4">
                <div className="space-y-6">
                  ...
                  {/* Products Display */}
                  {Object.entries(productsByName).length === 0 ? (
                    ...
                    <div className="space-y-4">
                      {filteredProducts.map((product) => {
                        ...
                        const displayName = productName ? (productName.name_en || productName.name) : 'Unknown Product';
                                  ...
                                  <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-2">
                                      ...
                                      <h4 className="font-semibold text-foreground group-hover:text-primary transition-colors">
                                        {product.compound_type_en?.trim() || product.compound_type?.trim() || 'Particularity'}
                                      </h4>
                                    </div>
                                    ...
                                    <div className="space-y-2">
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        ...
                                        {product.molecular_weight && product.molecular_weight > 0 && (
                                          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs bg-muted/15 text-muted-foreground rounded-full ring-1 ring-muted/25">
                                            <span>Molecular Weight: {product.molecular_weight} g/mol</span>
                                          </div>
                                        )}
                                      </div>
                                      ...
                                      <div className="flex flex-wrap gap-2">
                                        {product.cas_number && (
                                         ...
                                         {(product.cas_note_en || product.cas_note) && (
                                           <div className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs bg-orange-100 text-orange-800 rounded-full ring-1 ring-orange-200">
                                             <span>CAS Note: {product.cas_note_en || product.cas_note}</span>
                                           </div>
                                         )}
                                      </div>
                                    </div>
                                  ...
                                  <div className="grid md:grid-cols-2 gap-6">
                                             ...
                                             {product.applications.map((application) => application && (
                                               <Badge key={application.id} variant="secondary" className="px-3 py-1 font-medium text-foreground hover:bg-muted/50 transition-colors bg-transparent">
                                                 <div className="flex items-center gap-2">
                                                   ...
                                                   {application.name_en || application.name}
                                                 </div>
                                               </Badge>
                                             ))}
                                          </div>
                                    ...
                                    <div className="space-y-4">
                                       {product.product_image_url && (
                                         <div>
                                           <span className="text-sm font-medium text-foreground block mb-2">Product Image</span>
                                           <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                                             <img 
                                           ...
                                           <p className="text-xs text-muted-foreground mt-2 italic text-center">
                                             Image for illustration purposes only. For real product appearance and characteristics, please contact us.
                                           </p>
                                         </div>
                                       )}
                                       {product.molecular_structure_image_url && (
                                         <div>
                                           <span className="text-sm font-medium text-foreground block mb-2">Molecular Structure</span>
                                           <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                                             <img 
                                               src={product.molecular_structure_image_url} 
                                               alt={`Molecular structure of ${displayName}`}
                                               className="w-full h-auto rounded-xl object-contain"
                                               loading="lazy"
                                             />
                                           </div>
                    ...
                    // Grouped Mode - Show products grouped by name
                    <div className="space-y-6">
                      {Object.entries(productsByName).map(([nameId, { name, products }]) => (
                        <Card key={nameId} className="shadow-elegant ring-1 ring-border overflow-hidden">
                          <CardContent className="p-0">
                            ...
                            <div 
                              className="relative flex items-center justify-between p-6 cursor-pointer hover:bg-muted/50 transition-colors group"
                            ...
                            >
                              ...
                              <div className="flex items-center gap-4">
                                <div className="p-2 bg-primary/10 rounded-xl ring-1 ring-primary/20 group-hover:shadow-glow transition-all">
                                ...
                                <div className="flex items-center gap-3">
                                  ...
                                  <div>
                                    <h2 className="text-2xl font-bold text-foreground group-hover:text-primary transition-colors">
                                      {name.name_en || name.name}
                                    </h2>
                                    <p className="text-sm text-muted-foreground">
                                      {products.length} {products.length !== 1 ? 'variations' : 'variation'}
                                    </p>
                                  </div>
                                </div>
                              </div>
                                ...
                                <div className="grid gap-4">
                                  {products.map((product) => {
                                    ...
                                    return (
                                      ...
                                      {isProductExpanded && (
                                        <div className="border-t bg-gradient-card p-4">
                                          <div className="grid md:grid-cols-2 gap-6">
                                            {/* Product Information */}
                                            <div className="space-y-4">
                                              {/* Applications */}
                                               ...
                                               {product.product_image_url && (
                                                 <div>
                                                   ...
                                                   <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                                                     <img 
                                                       src={product.product_image_url} 
                                                       alt={`Product image of ${name.name_en || name.name}`}
                                                       className="w-full h-auto rounded-xl object-contain"
                                                       loading="lazy"
                                                     />
                                                   </div>
                                               ...
                                               {product.molecular_structure_image_url && (
                                                 <div>
                                                   ...
                                                   <div className="bg-card/80 backdrop-blur-sm rounded-2xl p-4 shadow-elegant ring-1 ring-primary/10">
                                                     <img 
                                                       src={product.molecular_structure_image_url} 
                                                       alt={`Molecular structure of ${name.name_en || name.name}`}
                                                       className="w-full h-auto rounded-xl object-contain"
                                                       loading="lazy"
                                                     />
                                                   </div>
        ...
        {/* Disclaimer Section */}
        <section className="bg-gradient-section-warm py-8 border-t border-border/50">
          <ResponsiveContainer className="text-center">
            <div className="space-y-4">
              <h3 className="text-xl font-semibold text-foreground">Need more information?</h3>
              <p className="text-muted-foreground max-w-2xl mx-auto">
                Our technical team composed of chemists, engineers and food technologists is always ready to answer your questions and provide detailed information about our products.
              </p>
              <Button 
                size="lg" 
                onClick={() => setContactOpen(true)}
