...
interface AccessRequestEmailRequest {
  requestId: string;
  adminEmail: string;
  adminName: string;
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const { 
      ...
      requestedDepartment,
      requestData
    }: AccessRequestEmailRequest = await req.json();
    ...
    await supabase.from('approval_tokens').insert([
      {
        ...
        action: 'approve',
        created_by: requestData.admin_id || '00000000-0000-0000-0000-000000000000'
      },
      {
        access_request_id: requestId,
    ...
    const roleLabel = roleLabels[requestedRole as keyof typeof roleLabels] || requestedRole;
...
<html lang="pt-BR">
  <head>
    ...
    <title>Nova Solicitação de Acesso – Aksell Nutrition</title>
    <style>
      :root {
  ...
  <body>
    <div class="wrapper">
      <div class="container">
        ...
        <div class="content">
          <div class="title">Nova Solicitação de Acesso</div>
          <p>Olá, <strong>${adminName}</strong>!</p>
          <p>Uma nova solicitação de acesso foi recebida e aguarda sua aprovação.</p>
          
          <div class="request-card">
            <div class="urgent-badge">Nova Solicitação</div>
            
            <div class="user-avatar">
              ${requesterName.charAt(0).toUpperCase()}
            </div>
            
            <h3 style="margin: 0 0 16px 0; color: var(--color-dark); text-align: center;">${requesterName}</h3>
            ...
            <div class="detail-row">
              <span class="detail-label">Cargo Solicitado:</span>
              <span class="detail-value">${roleLabel}</span>
            </div>
            
            <div class="detail-row">
              <span class="detail-label">Departamento:</span>
              <span class="detail-value">${requestedDepartment}</span>
            </div>
          ...
          <div class="button-container">
            <a href="${approveUrl}" class="button btn-approve">✓ Aprovar Acesso</a>
            <a href="${rejectUrl}" class="button btn-reject">✗ Rejeitar Solicitação</a>
          </div>
    ...
    const { error } = await resend.emails.send({
      from: 'Sistema de Acesso <noreply@aksell.com.br>',
      to: [adminEmail],
      subject: `🔐 Nova Solicitação de Acesso: ${requesterName} (${roleLabel})`,
      html: html,
    });
    ...
    console.log(`Email de solicitação de acesso enviado para ${adminEmail}`);

    return new Response(JSON.stringify({ 
      success: true, 
      message: 'Email de solicitação de acesso enviado com sucesso',
