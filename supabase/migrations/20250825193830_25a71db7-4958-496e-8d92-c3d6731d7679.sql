-- Verificar se o enum já existe e criar se necessário
DO $$ 
BEGIN
...
-- Adicionar colunas à tabela tasks se não existirem
DO $$ 
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tasks' AND column_name = 'fixed_type') THEN
        ALTER TABLE public.tasks ADD COLUMN fixed_type public.fixed_task_type NOT NULL DEFAULT 'simple_task';
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'tasks' AND column_name = 'payload') THEN
        ALTER TABLE public.tasks ADD COLUMN payload jsonb NOT NULL DEFAULT '{}';
    END IF;
END $$;
...
-- Recriar a tabela task_templates com a estrutura correta
