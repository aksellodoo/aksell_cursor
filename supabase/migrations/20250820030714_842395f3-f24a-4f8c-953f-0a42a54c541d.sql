-- Plano corrigido: Configurar workflows para tabelas Protheus (com correção de função)
...
-- 3) Configurar apenas tabelas Protheus dinâmicas válidas (que estão registradas)
DO $$
DECLARE
  rec RECORD;
...
BEGIN
  FOR rec IN 
    SELECT pdt.supabase_table_name
    FROM protheus_dynamic_tables pdt
    WHERE pdt.supabase_table_name IS NOT NULL
      AND EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' 
          AND table_name = pdt.supabase_table_name
          AND table_type = 'BASE TABLE'
      )
  LOOP
    PERFORM public.setup_protheus_table_workflow(rec.supabase_table_name);
    table_count := table_count + 1;
  END LOOP;
  
  RAISE NOTICE 'Configuração aplicada a % tabelas Protheus dinâmicas', table_count;
END $$;
...
-- 5) Dropar e recriar função process_workflow_triggers para logs mais claros
