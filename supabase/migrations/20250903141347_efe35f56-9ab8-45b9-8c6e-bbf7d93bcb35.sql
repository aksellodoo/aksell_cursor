...
-- Create a normalize_text function for consistent text comparison
...
-- Update the query_dynamic_table function to use normalize_text for better filtering
CREATE OR REPLACE FUNCTION public.query_dynamic_table(table_name_param text, search_term text DEFAULT NULL::text, column_filters jsonb DEFAULT NULL::jsonb, sort_column text DEFAULT NULL::text, sort_direction text DEFAULT 'asc'::text, limit_param integer DEFAULT 50, offset_param integer DEFAULT 0)
