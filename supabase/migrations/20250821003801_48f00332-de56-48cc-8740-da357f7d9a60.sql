...
-- [A] Desabilitar todos os EVENT TRIGGERS (para nada disparar durante a limpeza)
...
-- [B] Dropar EVENT TRIGGERS que referenciam a função OID = 59783
DO $$
DECLARE t record;
BEGIN
  FOR t IN
    SELECT evtname FROM pg_event_trigger WHERE evtfoid = 59783::oid
  LOOP
    EXECUTE format('DROP EVENT TRIGGER IF EXISTS %I', t.evtname); -- sem CASCADE
  END LOOP;
END $$;

-- [C] Dropar TRIGGERS DE TABELA (pg_trigger) que chamem a função por nome
DO $$
DECLARE trg record;
BEGIN
  FOR trg IN
...
-- [D] Dropar a função **por OID** (gera o nome com assinatura correta) usando RESTRICT
DO $$
DECLARE fn text;
BEGIN
...
-- [F] (Opcional) Hotfix SA3010: criar colunas + índice + backfill + recarregar schema
...
-- [G] Restaurar estado dos EVENT TRIGGERS
DO $$
