-- Create function to ensure soft delete columns exist (avoiding trigger conflict)
CREATE OR REPLACE FUNCTION public.add_soft_delete_columns(p_table_name text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.backfill_deletions_for_table(p_table_name text)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  SELECT public.add_soft_delete_columns(table_name) INTO columns_result;
  ...
  SELECT public.backfill_deletions_for_table(table_name) INTO backfill_result;
