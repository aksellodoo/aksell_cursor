...
-- Constraints CHECK para validação mínima (usando DO blocks para ser idempotente)
DO $$
BEGIN
  -- Constraint para form tasks
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'chk_tasks_form_has_form_id') THEN
    ALTER TABLE public.tasks
    ADD CONSTRAINT chk_tasks_form_has_form_id
    CHECK (fixed_type <> 'form' OR (payload ? 'form_id'));
  END IF;

  -- Constraint para approval tasks
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'chk_tasks_approval_min') THEN
    ALTER TABLE public.tasks
  ...
  -- Constraint para signature tasks
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'chk_tasks_signature_min') THEN
    ALTER TABLE public.tasks
  ...
  -- Constraint para email tasks
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'chk_tasks_email_min') THEN
    ALTER TABLE public.tasks
  ...
  -- Constraint para meeting tasks
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'chk_tasks_meeting_times') THEN
    ALTER TABLE public.tasks
  ...
  -- Constraint para workflow tasks
  IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'chk_tasks_workflow_id') THEN
    ALTER TABLE public.tasks
