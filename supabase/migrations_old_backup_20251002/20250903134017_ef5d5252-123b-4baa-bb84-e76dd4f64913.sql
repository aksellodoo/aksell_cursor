  ...
  RETURN QUERY EXECUTE format($q$
    ...
    groups AS (
      SELECT
        ...
        btrim(pcg.assigned_vendor_cod::text)    AS assigned_vendor_cod,
        btrim(coalesce(pcg.assigned_vendor_filial::text, '01')) AS assigned_vendor_filial
    ...
    group_vendor_resolved AS (
      SELECT
        g.id_grupo,
        coalesce(sa3g.a3_nreduz, sa3g.a3_nome, g.assigned_vendor_cod) AS group_vendor_name
      FROM groups g
      LEFT JOIN sa3_all sa3g
        ON sa3g.a3_cod    = g.assigned_vendor_cod
       AND sa3g.a3_filial = g.assigned_vendor_filial
    )
    SELECT
      g.id_grupo,
      g.group_id,
