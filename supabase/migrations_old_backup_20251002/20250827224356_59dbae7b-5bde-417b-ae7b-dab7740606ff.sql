...
BEGIN
  -- Obter nome da tabela dinâmica (SA1010 correspondente)
  ...
  -- Percorrer unidades (filial, cod, loja) ainda sem grupo
  ...
  LOOP
    -- Tentar localizar o grupo (capturando INTEGER e UUID)
    SELECT id_grupo, id
    ...
    -- Se não existir, criar o grupo e capturar id_grupo (int) e id (uuid)
    ...
    -- Vincular a unidade ao grupo (usa id_grupo INTEGER)
    ...
    -- Registrar resultado da execução usando UUID correto do grupo
    INSERT INTO public.protheus_group_update_results (
      ...
      v_group_uuid,  -- UUID real do grupo
  ...
  -- Contar membros ignorados (já em grupos)
  EXECUTE format($q$
    SELECT COUNT(*)
    FROM %I sa1
    WHERE EXISTS (
  ...
  -- Atualizar contagem de unidades por grupo
