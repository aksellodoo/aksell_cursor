-- Fix 1: Ensure update_protheus_customer_groups handles group_id as integer and commits transaction
...
BEGIN
  ...
  LOOP
    ...
    -- Assign the unit to the group using id_grupo (INTEGER, not UUID)
    INSERT INTO public.protheus_customer_group_units (
      ...
      v_unit.cod,
      v_unit.loja,
      v_group_id_grupo,  -- Use INTEGER directly
      auth.uid()
    );

    -- Log the action using id_grupo as INTEGER  
    INSERT INTO public.protheus_group_update_results (
      ...
      v_group_id_grupo::uuid,  -- Cast INTEGER to UUID for logging table
  ...
  -- EXPLICIT COMMIT to ensure transaction is saved
  COMMIT;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Rollback and re-raise with details
    ROLLBACK;
    RAISE EXCEPTION 'Erro em update_protheus_customer_groups: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
END;
$function$;

-- Fix 2: Correct list_group_units to use proper JOINs and handle id_grupo as INTEGER
CREATE OR REPLACE FUNCTION public.list_group_units(p_table_id uuid)
 RETURNS TABLE(filial text, cod text, loja text, nome text, group_id uuid, group_name text, assigned_at timestamp with time zone)
 LANGUAGE plpgsql
...
-- Fix 3: Correct get_last_group_update_results to use proper JOINs and handle group_id correctly
...
BEGIN
  ...
  RETURN QUERY EXECUTE format('
    ...
    INNER JOIN public.protheus_customer_groups pcg ON pcg.id = pgur.group_id
    WHERE pgur.run_id = %L
