...
-- Function to automatically add record_status and triggers to existing protheus tables
DO $$
DECLARE
  table_rec RECORD;
  ...
  trigger_name TEXT;
BEGIN
  -- Loop through all protheus tables
  FOR table_rec IN 
    SELECT table_name 
    FROM information_schema.tables 
    WHERE table_schema = 'public' 
    AND table_name LIKE 'protheus_%'
    AND table_name != 'protheus_tables'
  LOOP
    table_full_name := 'public.' || table_rec.table_name;
    trigger_name := 'trigger_emit_protheus_status_change_' || REPLACE(table_rec.table_name, 'protheus_', '');
    ...
    IF NOT EXISTS (
      ...
      EXECUTE format('
        CREATE TRIGGER %I
          ...
          FOR EACH ROW
          EXECUTE FUNCTION public.emit_protheus_status_change()', trigger_name, table_rec.table_name);
      
      RAISE NOTICE 'Added trigger % to %', trigger_name, table_rec.table_name;
    END IF;
  END LOOP;
END $$;