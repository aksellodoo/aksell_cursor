...
-- 1) Fortalecer a função que emite eventos ao detectar mudanças no Protheus
...
BEGIN
  ...
  IF should_emit THEN
    -- Log diagnóstico para facilitar troubleshooting e auditoria
    ...
    -- Disparar processamento de workflows
...
-- 2) Criar (ou recriar) triggers no(s) nome(s) reais das tabelas SA3010
DO $$
DECLARE
  rec RECORD;
  trg_main text;
BEGIN
  FOR rec IN
    ...
    WHERE table_schema = 'public'
      AND table_name LIKE 'protheus_sa3010_%'
  LOOP
    trg_main := rec.table_name || '_status_change_trigger';

    -- Remover triggers antigos com nomes anteriores que possam conflitar
    ...
    EXECUTE format('DROP TRIGGER IF EXISTS %I ON public.%I', 'protheus_sa3010_fc3d70f6_status_change_trigger', rec.table_name);

    -- Criar trigger com nome determinístico baseado no nome real da tabela
    EXECUTE format(
      'CREATE TRIGGER %I
