-- A. Kill the event trigger by OID (no DDL, no subselects)
-- Replace 59784 with the actual event trigger OID you saw (the dep of func OID 59783).
BEGIN;
DELETE FROM pg_event_trigger WHERE oid = 59784;
COMMIT;

-- B. Drop the function by exact signature (RESTRICT)
-- If unsure of the signature:
SELECT oid::regprocedure::text FROM pg_proc WHERE proname='auto_setup_protheus_table';
-- then:
DROP FUNCTION IF EXISTS public.auto_setup_protheus_table() RESTRICT;

-- C. Sanity checks (should be empty)
