...
BEGIN
    ...
    -- Simular a descoberta dos campos do Oracle (baseado nos campos que já existem na tabela física)
    -- Vamos coletar todos os campos da tabela física exceto os de controle
    ...
    -- Reconstituir o mapeamento original baseado na convenção de nomes
    ...
    IF jsonb_array_length(oracle_fields) > current_count THEN
        RAISE NOTICE 'Encontrados % campos a mais que precisam ser adicionados', 
                     jsonb_array_length(oracle_fields) - current_count;
        ...
        RAISE NOTICE 'Mapeamento atualizado com % campos totais', jsonb_array_length(oracle_fields);
        
        -- Log da correção
        INSERT INTO field_audit_log (record_id, field_name, old_value, new_value, changed_by, record_type)
        VALUES (
            protheus_table_id_var,
            'field_mappings_corrected',
            current_count::text,
            jsonb_array_length(oracle_fields)::text,
