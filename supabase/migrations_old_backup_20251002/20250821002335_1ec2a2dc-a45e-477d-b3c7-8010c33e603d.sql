...
-- 1) Snapshot & disable ALL event triggers (so no code runs while we clean)
...
-- 3) Drop event triggers that point to those functions (by evtfoid and by definition text)
DO $$
DECLARE t record;
BEGIN
  FOR t IN
  ...
  END LOOP;
  FOR t IN
    SELECT et.evtname FROM pg_event_trigger et
...
-- 3.5) Drop TABLE triggers (pg_trigger) que usam a função auto_setup_protheus_table
DO $$
DECLARE trg record;
BEGIN
  FOR trg IN
