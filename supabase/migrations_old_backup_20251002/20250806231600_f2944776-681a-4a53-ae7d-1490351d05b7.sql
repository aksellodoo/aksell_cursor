...
-- 6. Fix search_path on existing security-critical functions (if they exist)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'get_department_name') THEN
    ALTER FUNCTION public.get_department_name(uuid) SET search_path TO 'public';
  END IF;
  
  IF EXISTS (SELECT 1 FROM pg_proc WHERE proname = 'process_access_request_approval') THEN
