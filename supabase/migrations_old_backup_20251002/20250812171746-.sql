-- Security hardening migration (fix policy drop blocks)

-- 1) Ensure external login attempts table exists (used by external-form-auth)
...
-- 2) Lock down sensitive tables (drop all existing client policies and revoke anon/authenticated access)
...
BEGIN
  ...
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.password_reset_tokens', pol.policyname);
  END LOOP;
END$$;
DO $$BEGIN
...
BEGIN
  ...
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.form_external_sessions', pol.policyname);
  END LOOP;
END$$;
DO $$BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='form_external_sessions') THEN
...
BEGIN
  ...
  LOOP
    EXECUTE format('DROP POLICY IF EXISTS %I ON public.approval_tokens', pol.policyname);
  END LOOP;
END$$;
DO $$BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='approval_tokens') THEN
...
-- 4) Enforce server-side throttling for pending access requests
