...
-- Update group_id values in protheus_customer_group_units to match the new id_grupo
UPDATE protheus_customer_group_units 
SET group_id = pcg.id_grupo::text::uuid
FROM protheus_customer_groups pcg
WHERE protheus_customer_group_units.group_id = pcg.id;

-- Update group_id values in protheus_group_update_results to match the new id_grupo
UPDATE protheus_group_update_results 
SET group_id = pcg.id_grupo::text::uuid
FROM protheus_customer_groups pcg
WHERE protheus_group_update_results.group_id = pcg.id;

-- Change group_id column type from UUID to INTEGER in protheus_customer_group_units
ALTER TABLE protheus_customer_group_units 
ALTER COLUMN group_id TYPE INTEGER USING group_id::text::integer;

-- Change group_id column type from UUID to INTEGER in protheus_group_update_results  
ALTER TABLE protheus_group_update_results 
ALTER COLUMN group_id TYPE INTEGER USING group_id::text::integer;

-- Now drop the old primary key from protheus_customer_groups
...
DECLARE
  v_table TEXT;
  v_vendor_table TEXT := 'sa3010_vendedores';
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = v_vendor_table) THEN
    v_vendor_table := NULL;
  END IF;
  ...
  RETURN QUERY EXECUTE format($q$
    SELECT 
      ...
      ARRAY_AGG(DISTINCT 
        CASE 
          WHEN %L IS NOT NULL AND sa3.a3_nome IS NOT NULL THEN sa3.a3_nome::text 
  ...
  $q$, v_vendor_table, v_table, 
       CASE WHEN v_vendor_table IS NOT NULL 
            THEN 'LEFT JOIN ' || v_vendor_table || ' sa3 ON sa3.a3_cod::text = sa1.a1_vend::text'
            ELSE ''
       END,
       p_table_id);
END;
